...
namespace Catch {
    ...
    public:
        ...
        void useStream( std::string const& streamName ) {
            Stream stream2 = createStream( streamName );
            setStreamBuf( stream2.streamBuf );
            m_stream.release();
            m_stream = stream2;
        }
...
#  endif
    ...
    class CommandLine {
        ...
        public:
            ...
            ArgBuilder& detail( std::string const& detail2 ) {
                m_arg->detail = detail2;
                return *this;
            }
    ...
    public:
        ...
        void optUsage( std::ostream& os, std::size_t indent = 0, std::size_t width = Detail::consoleWidth ) const {
            ...
            for( it = itBegin; it != itEnd; ++it ) {
                Detail::Text usage2( it->commands(), Detail::TextAttributes()
                                                        .setWidth( maxWidth+indent )
                                                        .setIndent( indent ) );
                Detail::Text desc( it->description, Detail::TextAttributes()
                                                        .setWidth( width - maxWidth - 3 ) );

                for( std::size_t i = 0; i < (std::max)( usage2.size(), desc.size() ); ++i ) {
                    std::string usageCol = i < usage2.size() ? usage2[i] : "";
                    os << usageCol;
...
namespace Catch {
    ...
    public:
        ...
        explicit RunContext( Ptr<IConfig const> const& config2, Ptr<IStreamingReporter> const& reporter )
	    :   m_runInfo( config2->name() ),
